            ...
            rows.into_iter()
                .map(|r| {
                    ...
                    json!({
                        ...
                        "config": m.as_ref().map(|m| m.config.iter().map(|c| json!({
                            "key": c.key.clone(),
                            "label": c.label.clone(),
                            "required": c.required,
                            "hint": c.hint.clone(),
...
/// 读库失败会被 `Db::get` 一并吞成「无值」,所以库故障时这里也会报缺配置而非 500。
fn missing_required_config(app: &App, plugin_id: &str, manifest: &Manifest) -> Vec<String> {
    manifest
        .config
        .iter()
        .filter(|c| c.required)
        .filter(|c| app.db.get(&format!("plugin.{plugin_id}:{}", c.key)).is_none_or(|v| v.trim().is_empty()))
        .map(|c| match c.label.as_deref() {
            Some(label) if !label.trim().is_empty() => format!("{label}({})", c.key),
            _ => c.key.clone(),
        })
        .collect()
}
...
///
/// 派发前先按 manifest 的 `[[config]]` 预检必填项:插件返回 `other:2` 这种码,
...
pub async fn test_plugin(_: Admin, State(app): State<Shared>, Path(id): Path<i64>) -> Response {
    ...
    if let Some(manifest) = &manifest {
        let missing = missing_required_config(&app, &plugin_id, manifest);
        if !missing.is_empty() {
            return bad(&format!(
...
///
/// 长度上限单点在 [`plugin::KV_KEY_MAX`]:manifest 的 `[[config]]` 校验用的是
/// 同一个数,两处各写一份迟早漂。
fn kv_key_error(key: &str) -> Option<Response> {
    if key.trim().is_empty() {
        return Some(bad("key 不能为空"));
    }
    if key.len() > plugin::KV_KEY_MAX {
        return Some(bad(&format!("key 超过 {} 字节的上限", plugin::KV_KEY_MAX)));
    }
    if key.contains(':') {
...
mod tests {
    ...
    /// 列表接口把 manifest 的 `[[config]]` 透给面板:key/label/required/hint,
    ...
    async fn a_plugin_declares_its_config_for_the_panel() {
        ...
        // 没声明 [[config]] 的插件解析出空表:面板照旧,不显示配置提示。
        assert_eq!(
    ...
    async fn a_plugin_kv_row_can_be_deleted_on_its_own() {
        ...
        for bad_key in ["   ", "a:b", &"k".repeat(plugin::KV_KEY_MAX + 1)] {
